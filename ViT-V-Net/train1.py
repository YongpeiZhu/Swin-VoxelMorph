...
from Functions import generate_grid, Dataset_epoch_crop, Predict_dataset_crop, CompositionTransform
...
def main():
    ...
    # model = SwinUnet(config, img_size=224, num_classes=3)
    if cont_training:
        epoch_start = 335
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            loss = 0
            loss_vals = []
            curr_loss_sim = criterions[0](output[0], y) * weights[0]+criterions[0](output[1], x) * weights[0]
            # curr_loss2 = 
            loss_vals.append(curr_loss_sim)
            curr_loss_grad = criterions[1](output[2], y) * weights[1]+criterions[1](output[3], x) * weights[1]
            loss_vals.append(curr_loss_grad)
            curr_loss_Jdet = losses.neg_Jdet_loss(output[2], grid) * 1000+losses.neg_Jdet_loss(output[3], grid) * 1000
            loss_vals.append(curr_loss_Jdet)
            F_X_Y = com_transform(output[2], output[3], grid, range_flow)
            curr_loss_cons_pair=10*torch.mean((F_X_Y-grid)**2)
            loss += curr_loss_sim+curr_loss_grad+curr_loss_Jdet+curr_loss_cons_pair
            ...
            print('Iter {} of {} loss {:.4f}, Img Sim: {:.6f}, Reg: {:.6f}, Jdet: {:.6f}'.format(idx, len(train_loader), loss.item(), loss_vals[0].item()/2, loss_vals[1].item()/2,loss_vals[2].item()/2))
